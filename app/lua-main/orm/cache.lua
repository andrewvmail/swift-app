...
_cache = {
    ...
    initWithDB = function (dbname)
        if _dbThreadId == -1 then
            ...
            lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","init",dbname)
        end
    end,

    query = function (query, params, name)
        params = params or {}
        local data = lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","query",query,params,name)
        return data
    end,

    execute = function (query, params)
        params = params or {}
        local data = lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","execute",query,params)
        return data
    end,
    ...
    rows = function (tablename, sql, bindValues, needColumns, whereIds, selectColums)
        ...
        results = lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","rows",sql,bindValues)
    ...
    insert = function (tablename, kv, needPrimaryKey)
        ...
        if not needPrimaryKey then
            lua_thread.postToThread(_dbThreadId,"lua-main.orm.model","insert",insert,bindValues)
            _cache._saveKVtoCache(tablename,kv)
        else
            local result,rowId
            result,rowId = lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","insert",insert,bindValues,needPrimaryKey)
    ...
    batchInsert = function(tablename, params)
        ...
        if allNoNeedPrimaryKey then
            lua_thread.postToThread(_dbThreadId,"lua-main.orm.model","batchInsert",saveParams)
        else 
            result = lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","batchInsert",saveParams)
        end
        for i, param in ipairs(params) do
            local kv = param["kv"];
            local needPrimaryKey = param["needPrimaryKey"];
    ...
    update = function (tablename, whereSql, whereBindingValues, data)
        ...
        if t.__primary_key then
            ...
            local results = lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","rows",selectSql,whereBindingValues)
            local ids = {}
            ...
            if #ids > 0 then
                local s = require('lua-main.orm.class.select')(t)
                s:primaryKey(ids)
                ...
                lua_thread.postToThread(_dbThreadId,"lua-main.orm.model","execute",_update,setBindingValue)
                -- remove cache
    ...
    deleteAll = function (tablename)
        ...
        lua_thread.postToThread(_dbThreadId,"lua-main.orm.model","execute",sql)
    ...
    delete = function (tablename, whereSql, bindValues)
        ...
        if t.__primary_key then
            local selectSql = "SELECT "..t.__primary_key.name.." from "..tablename.." "..whereSql
            local results = lua_thread.postToThreadSync(_dbThreadId,"lua-main.orm.model","rows",selectSql,bindValues)
            local ids = {}
            for _,v in ipairs(results) do
                table.insert(ids, v[1])
            end
            if #ids > 0 then
                ...
                lua_thread.postToThread(_dbThreadId,"lua-main.orm.model","execute",sql,ids)
            ...
            local sql = "DELETE from "..tablename.." "..whereSql
            lua_thread.postToThread(_dbThreadId,"lua-main.orm.model","execute",sql,bindValues)
        end
    end,
