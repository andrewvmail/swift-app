...
local Select = function(own_table)
    return {
        ...
        _select = function (self)
            ...
            return lua_thread.postToThreadSync(self.own_table.cacheThreadId,"lua-main.orm.cache","rows",self.own_table.__tablename__,_select,self._rules._bindValuse,needColumns,self._rules.primaryKey,self._rules.selectColumns)
        end,
        ...
        update = function (self, data)
            ...
            if Type.is.table(data) then
                if self._rules.primaryKey and #self._rules.primaryKey>0 then
                    lua_thread.postToThread(self.own_table.cacheThreadId,"lua-main.orm.cache","updateWithPrimaryKey",self.own_table.__tablename__,self._rules.primaryKey,data)
                else
                    -- -- Build WHERE
                    local _where = ""
                    ...
                    lua_thread.postToThread(self.own_table.cacheThreadId,"lua-main.orm.cache","update",self.own_table.__tablename__,_where,self._rules._bindValuse,data)
                end
            else
        ...
        delete = function (self)
            ...
            if deleteWithPrimary then
                lua_thread.postToThread(self.own_table.cacheThreadId,"lua-main.orm.cache","deleteWithPrimaryKey",self.own_table.__tablename__,self._rules.primaryKey)
            elseif deleteAll == false then
                lua_thread.postToThread(self.own_table.cacheThreadId,"lua-main.orm.cache","delete",self.own_table.__tablename__,_delete,self._rules._bindValuse)
            else
                lua_thread.postToThread(self.own_table.cacheThreadId,"lua-main.orm.cache","deleteAll",self.own_table.__tablename__)
            end
        end,
